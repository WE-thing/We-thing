...
export default function Chat() {
  ...
  return (
    <div className=" w-full relative h-full overflow-y-hidden">
      <div
        ref={messageListRef}
        className={`w-full h-[calc(80vh-56px)] overflow-y-auto`}
      >
        {messageList.map((message, id) => (
          <ChatMessage key={id} message={message} />
        ))}
      </div>
      <div className=" w-100 h-[56px] mx-auto absolute bottom-0 grid grid-cols-6 bottom-0 h-14 bg-red-50 shadow-[rgba(0,0,15,0.1)_0px_0px_14px_0px]">
        <input
          type="text"
          ref={messageInputRef}
          placeholder="메세지 입력"
